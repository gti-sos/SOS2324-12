...
const API_BASE_JMS = "/api/v3/airbnb-listings";
...
function loadBackend_JMS_v3(app, firebaseApp) {
    ...
    app.get(API_BASE_JMS + "/", async (req, res) => {
        ...
        try {
            ...
            res.status(200).send(listingsData);
        } catch (error) {
    ...
    app.get(API_BASE_JMS+"/loadInitialData", async (req, res) => {
      try {
          const listingsRef = collection(db, 'listings');
          const querySnapshot = await getDocs(listingsRef);
          if (querySnapshot.empty) {
              for (const data of initialData) {
                  await addDoc(listingsRef, data);
              }
          ...
          console.error("Error fetching or inserting initial data:", error);
          res.status(500).send("Internal Server Error");
      }
  });
    ...
    app.get(API_BASE_JMS + "/:latitude/:longitude", async (req, res) => {
        const { latitude, longitude } = req.params;
        
        
        try {
            const listingsRef = collection(db, 'listings');
            const q = query(listingsRef, where("latitude", "==", parseFloat(latitude)), where("longitude", "==", parseFloat(longitude)));
            const querySnapshot = await getDocs(q);
            const listingsData = querySnapshot.docs.map(doc => doc.data());
            res.status(200).send(listingsData[0]);
        } catch (error) {
            console.error("Error fetching data from Firebase:", error);
            res.status(500).send("Internal Server Error");
        }
    ...
    app.post(API_BASE_JMS + "/", async (req, res) => {
        ...
        if (!isValidData) {
            ...
            try {
                const listingsRef = collection(db, 'listings');
                const querySnapshot = await getDocs(query(listingsRef, where("latitude", "==", newData.latitude), where("longitude", "==", newData.longitude)));
                if (!querySnapshot.empty) {
    ...
    app.put(API_BASE_JMS + "/:latitude/:longitude", async (req, res) => {
        ...
        if (!data || Object.keys(data).length === 0 || data.latitude !== latitude || data.longitude !== longitude) {
            ...
            try {
                const listingsRef = collection(db, 'listings');
                const querySnapshot = await getDocs(query(listingsRef, where("latitude", "==", latitude), where("longitude", "==", longitude)));
                if (querySnapshot.empty) {
                    res.status(404).send("Listing not found");
                } else {
                    const docRef = querySnapshot.docs[0].ref;
                    await updateDoc(docRef, data);
                    res.status(200).send("OK, data updated");
                }
            } catch (error) {
    ...
    app.delete(API_BASE_JMS + "/", async (req, res) => {
      try {
        const listingsRef = collection(db, 'listings'); // Assuming your collection name is 'listings'
        const querySnapshot = await getDocs(listingsRef);
    
        const batch = db.batch(); // Create a WriteBatch object
        ...
        res.status(200).send("OK, All data has been deleted");
      } catch (error) {
    ...
    app.delete(API_BASE_JMS + "/:latitude/:longitude", async (req, res) => {
        const latitude = parseFloat(req.params.latitude);
        const longitude = parseFloat(req.params.longitude);
